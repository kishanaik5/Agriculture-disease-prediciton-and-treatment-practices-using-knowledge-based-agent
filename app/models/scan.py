...
class PaymentTransaction(BaseSchema):
    ...
    coupon_used = Column(String(50), nullable=True)


class TranslatedAnalysisReport(BaseSchema):
    __tablename__ = "translated_analysis_report"
    __table_args__ = {'schema': settings.DB_SCHEMA}

    user_id = Column(String(50), nullable=False, index=True)
    report_uid = Column(String, nullable=True, index=True) # Logical ref to original report
    language = Column(String(10), nullable=True)
    category_type = Column(String(20), nullable=False) # crop, fruit, vegetable
...
class KnowledgeBase(BaseSchema):
    ...
    category = Column(String(50), nullable=False, index=True) # crop, fruit, vegetable
    item_name = Column(String(255), nullable=False, index=True) # Plant/Fruit/Veg Common Name
    disease_name = Column(String(255), nullable=False, index=True) # Disease Name (Type)
    scientific_name = Column(String(255), nullable=True) 
    treatment = Column(Text, nullable=True) # Treatment Methods
    language = Column(String(10), default="en")


class ReportAmount(BaseSchema):
    __tablename__ = "report_amount"
    __table_args__ = {'schema': settings.DB_SCHEMA}

    category = Column(String(50), nullable=False, unique=True, index=True) # crop, fruit, vegetable
    amount = Column(Float, nullable=False, default=0.0)


class MasterIcon(BaseSchema):
    __tablename__ = "master_icons"
    __table_args__ = {'schema': settings.DB_SCHEMA}
